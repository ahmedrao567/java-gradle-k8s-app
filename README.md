...
ISSUES FACED:
Alot of issues i have faced in configuring the postgresql deployment using helm charts
Then i take ai and web help to lookout for those issues that are basically the configuarion in build.gradle file 
the issue was including the jdbc postgresql dependencies in application.properties and buuld.gradle

the next issue i faced that my prometheus didnt configuring with my app the target was not shwoing
then i take a help from ai and web
the created a service monitor 
and re run the prometeus on localhost it showed the target

In jenkins shared libs i also faced issue in creating a shared libararies 
then i take concept from ai youtube and web 
then configure the stages in jenkins files
